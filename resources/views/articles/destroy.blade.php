<form method="POST" action="{{ $route }}" id="deleteForm{{ $article->id }}">
    @csrf
    @method('DELETE')
    <td>
        <button type="button" onclick="confirmDelete({{ $article->id }})" class="text-white bg-blue-700 hover:bg-blue-800
